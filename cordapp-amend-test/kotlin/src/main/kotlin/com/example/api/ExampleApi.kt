...
import com.example.flow.AmendFlow.Amendment
...
class ExampleApi(val services: CordaRPCOps) {
    ...
    @POST
    @Path("{party}/amend-purchase-order")
    fun updatePurchaseOrder(purchaseOrder: PurchaseOrder, @PathParam("party") partyName: String): Response {
        val otherParty = services.partyFromName(partyName)
        if (otherParty == null) {
            return Response.status(Response.Status.BAD_REQUEST).build()
        }

        val result: AmendFlowResult = services
                .startFlow(::Amendment, purchaseOrder, otherParty)
                .returnValue
                .toBlocking()
                .first()

        when (result) {
            is AmendFlowResult.Success ->
                return Response
                        .status(Response.Status.CREATED)
                        .entity(result.message)
                        .build()
            is AmendFlowResult.Failure ->
                return Response
                        .status(Response.Status.BAD_REQUEST)
                        .entity(result.message)
                        .build()
